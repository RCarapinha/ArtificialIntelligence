...
def main(argv):
    ...
    for mode in ['Home', 'Away']:
        feature_columns = feature_columns + [
            ...
            tf.feature_column.numeric_column(key='{}-OppositionGoals'.format(mode)),
            tf.feature_column.numeric_column(key='{}-Shots'.format(mode)),
            tf.feature_column.numeric_column(key='{}-ShotsOnTarget'.format(mode)),
            tf.feature_column.numeric_column(key='{}-OppositionShots'.format(mode)),
            tf.feature_column.numeric_column(key='{}-OppositionShotsOnTarget'.format(mode)),
        ]
